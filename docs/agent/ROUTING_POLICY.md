...
- `deprecated` → **блок‑ответ** с инструкцией (указать замену или переформулировать запрос).
- `ambiguous` → структурированный decision‑пакет с выбором навыка (`select_skill`), без исполнения.

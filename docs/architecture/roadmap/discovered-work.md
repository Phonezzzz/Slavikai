...
- **Проблема:** consumer ChatGPT/OpenAI account/subscription access нельзя считать
  ...
  automation.
- **Research scope:**
  - official account/subscription authentication и account ownership;
  - subscription entitlement, quota и достоверный способ определения eligibility;
  - permitted and supported access paths по authoritative product documentation;
  - явное отделение от OpenAI API, используемого для API-billed services;
  - quota exhaustion, expiry, revocation, challenge и recovery behavior;
  - explicit/non-silent fallback и запрет скрытого перехода между billing, trust,
    privacy, capability или data-egress boundaries;

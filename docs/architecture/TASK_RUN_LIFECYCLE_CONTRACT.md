...
**Статус: accepted target architecture; не описание current runtime (owner
acceptance 24.09.2026).** Основа — сохранённый snapshot `6040640`; semantic
redline и остающиеся вопросы находятся в
...
Current-runtime audit и его пределы находятся в
[`research/current-state-flow-ownership-matrix.md`](research/current-state-flow-ownership-matrix.md).
...
[`ADR-0010`](decisions/ADR-0010-supersession-lineage-without-separate-final.md) и
[`ADR-0012`](decisions/ADR-0012-ordinary-ask-task-identity.md).
...
MWV, Auto, coordinated multi-agent и background execution. Это не делает все
execution runs полностью checkpointed, background или multi-agent по умолчанию:
policy/capability выбирает необходимый execution mode,
но одинаковые identity, transition authority, completion и safety semantics действуют везде.
По ADR-0012 каждый принятый Ask имеет сохраняемые task identity, revision и
semantic final; полный durable execution checkpoint включается выборочно.
...
- **aborted** — administrative/policy/recovery stop без domain failure assertion.

Terminal causes for a user-facing terminal task revision are exactly
`completed / failed / cancelled / aborted` (ADR-0002). Each terminal cause carries a result
disposition `full / accepted_partial / none`, bound to exact task/criteria/result/artifact
revisions. `aborted` не переименовывается в `failed`: administrative/policy/recovery stop
сохраняет свою причину.

`superseded` **не** является пятой terminal cause. Это status/lineage transition (ADR-0010):
новая accepted revision закрывает старую для дальнейшего исполнения и связывает old/new
revision IDs без отдельного final старой revision только из-за замены. Различаются два случая:

- replacement внутри одного `task_id` — новая accepted revision того же logical task;
- explicit cross-task lineage — goal заменена другой logical task.

Обе формы фиксируют identity, actor/основание и status transition; ни одна не публикует final
старой revision по факту замены. Уже доставленный final и его evidence/artifact refs
сохраняются как исторический факт.

`completed`, `failed`, `cancelled`, `aborted` terminal для данной accepted task revision и не
...
| Verifier | Typed evidence outcome `passed/failed/inconclusive/stale/not_applicable/blocked/verifier_error`; не task completion сам по себе |
...
| `waiting_verification` | verifier outcome | Accept, rework, fail, escalate или STOP |
...
## 13. Completion, acceptance и verification
...
Semantic flow (verification отделена от acceptance и от task-revision terminal decision):

~~~text
work outcome -> result_submitted
  -> required verification attempt
       outcome: passed | failed | inconclusive | stale | not_applicable | blocked | verifier_error
  -> verification resolution (accept / rework / fail / escalate / STOP)
  -> final aggregation candidate
  -> authoritative acceptance transaction (full | accepted_partial | none)
  -> run transition (completed | failed | cancelled | aborted)
  -> task-revision terminal decision
  -> final readiness and delivery
~~~

Verification может быть optional по accepted plan/policy, но acceptance decision обязательна.
`result_submitted` и verifier `passed` сами не производят `completed` или final.

Verifier outcomes typed и attributed (Verification contract): `passed`, `failed`,
`inconclusive`, `stale`, `not_applicable`, `blocked`, `verifier_error`. `inconclusive`,
`stale`, `blocked` и `verifier_error` не трактуются как `passed` и не дают acceptance
автоматически.

Acceptance transaction отделена от run transition: она выбирает result disposition
`full / accepted_partial / none`, привязанный к exact task/criteria/result/artifact revisions.
Только authorized user или criteria, явно pre-approved этим user до execution, могут принять
incomplete scope. Budget exhaustion, model prose, artifact existence, verifier pass, policy
block или worker report не дают acceptance.

Completion transaction проверяет применимые criteria:

- current task/plan revisions и required subtasks;
- required verifier outcome и его revision;
- artifact/evidence integrity;
...
- result disposition согласована с terminal cause (см. §22).

Verifier rejection/failure создаёт rework/new attempt или terminal run failure по policy; logical
task может остаться active. Completion acknowledgement idempotent и fenced against
...
- final response — projection committed terminal state.

Final mapping (ADR-0002): semantic final несёт и terminal cause, и result disposition.
Presentation leads with the cause для `failed`, `cancelled` и `aborted`; accepted partial work
discloses отдельно. Одна task revision получает ровно один logical final.

| Terminal cause | Disposition | Communication class |
| --- | --- | --- |
| `completed` | `full` | `final_success` |
| `completed` | `accepted_partial` | `final_partial` |
| `failed` | `none` или `accepted_partial` | `final_failure`; disclose accepted work if present |
| `cancelled` | `none` или `accepted_partial` | `final_cancelled`; disclose accepted work if present |
| `aborted` | `none` или `accepted_partial` | `final_aborted`; disclose accepted work if present |

`completed + none` и non-completed `+ full` — invalid комбинации до явной reconciliation.
`final_partial` — communication class для accepted partial completion, не lifecycle state.
`superseded` не публикует отдельный final старой revision (ADR-0010).
...
Ранее открытые решения зафиксированы в ADR: terminal cause/disposition и accepted partial —

...
class RenameFileTool:
    def handle(self, request: ToolRequest) -> ToolResult:
        ...
        try:
            ...
            if old_path.is_file():
                if not _is_allowed_workspace_file(old_path):
                    return ToolResult.failure("Расширение файла запрещено для переименования.")
                if not _is_allowed_workspace_file(new_path):
                    return ToolResult.failure("Целевой файл должен иметь разрешённое расширение.")
            if new_path.exists():
    ...
    def handle(self, request: ToolRequest) -> ToolResult:
        ...
        try:
            ...
            if not source.exists():
                return ToolResult.failure("Исходный путь не найден.")
            if source.is_file():
                if not _is_allowed_workspace_file(source):
                    return ToolResult.failure("Расширение файла запрещено для перемещения.")
                if not _is_allowed_workspace_file(target):
                    return ToolResult.failure("Целевой файл должен иметь разрешённое расширение.")
            if target.exists():
                return ToolResult.failure("Целевой путь уже существует.")

...
def test_safe_mode_requires_approval_and_blocks_tool() -> None:
    ...
    with pytest.raises(ApprovalRequired):
        gateway.call(ToolRequest(name="shell", args={"command": "echo hi"}))
...
def test_approved_category_allows_tool() -> None:
    ...
    result = gateway.call(ToolRequest(name="shell", args={"command": "echo hi"}))
...
def test_hard_unsafe_command_blocked_even_without_safe_mode() -> None:
    """Hard safety floor: rm -rf / is blocked even when safe_mode is disabled."""
    registry = ToolRegistry()
    called = {"ok": False}
...
def test_hard_unsafe_command_blocks_before_approval_in_safe_mode() -> None:
    """Hard safety floor takes precedence over safe_mode approval flow."""
    registry = ToolRegistry()
    called = {"ok": False}
    ...
    result = gateway.call(ToolRequest(name="shell", args={"command": "sudo reboot"}))
    assert result.ok is False
    assert "запрещена политикой безопасности" in (result.error or "")
    assert called["ok"] is False

...
def _canonical_shell_config_path() -> Path:
    """Канонический путь конфига; разрешается динамически для тестов."""
...
def load_shell_config() -> ShellConfig:
    cfg_path = _canonical_shell_config_path()
...
def save_shell_config(config: ShellConfig) -> None:
    cfg_path = _canonical_shell_config_path()
    cfg_path.parent.mkdir(parents=True, exist_ok=True)
    try:

...
def _chat_id_from_request(request: web.Request) -> str:
...
async def handle_ui_status(request: web.Request) -> web.Response:
    ...
    requested_session_id = _extract_ui_session_id(request)
    session_id = await hub.get_or_create_session(requested_session_id, principal_id=principal_id)
...
async def handle_ui_mode(request: web.Request) -> web.Response:
    ...
    if next_mode == "ask":
        ...
        await hub.set_session_workflow(chat_id, mode=next_mode)
    updated = await hub.get_session_workflow(chat_id)
    response = json_response(
        {
            "ok": True,
            "chat_id": chat_id,
...
async def handle_ui_runtime_init(request: web.Request) -> web.Response:
    ...
    workspace_root_raw = await hub.get_workspace_root(chat_id)
    workspace_root = Path(workspace_root_raw) if isinstance(workspace_root_raw, str) else Path(".")
    workflow_before = await hub.get_session_workflow(chat_id)
    ...
    await hub.set_session_decision(chat_id, None)
    await hub.set_session_status(chat_id, "ok")
    ...
    if agent is not None:
        ...
        approved_categories = await session_store.get_categories(chat_id)
        try:
            await _apply_agent_runtime_state(agent=agent, hub=hub, session_id=chat_id)
            agent.set_session_context(chat_id, approved_categories)
    ...
    workflow = await hub.get_session_workflow(chat_id)
    tools_state, policy = await _load_effective_session_security(hub=hub, session_id=chat_id)
    response = json_response(
        {
            "ok": True,
            "chat_id": chat_id,
